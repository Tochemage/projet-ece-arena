...
{
    int cmpt=0, imnum, desc_num, desc_active=0, mouse_prev=1;
    BITMAP *buffer, *ninja, *tank, *wizard, *support, *desc[4];
    ...
    while(!key[KEY_ENTER])
    {
        ...
        {
            masked_blit(desc[desc_num], buffer, 0,0,120,100,desc[desc_num]->w,desc[desc_num]->h);
